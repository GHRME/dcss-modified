...
void level_change(bool skip_attribute_increase)
{
           ...
           && you.experience >= exp_needed(you.experience_level + 1))
    {
        ...
        else  // Character has gained a new level
        {
            ...
            if (new_exp == 99)
                mprf(MSGCH_INTRINSIC_GAIN, "You have reached level 99, the final one!");
            ...
            const bool manual_stat_level = true; // every level gives a stat point (modified)
            // original: only every 3rd level (% 3 == 0 or % 6 == 3)
